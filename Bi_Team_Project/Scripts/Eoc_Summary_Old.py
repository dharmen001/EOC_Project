...
class Summary (object):
	...
	merging columns
		...
		try:
			display_first__summary = self.read_sql__display.merge (self.read_sql__display__m_v, on="PLACEMENT#", how="inner",
			                                                suffixes=('_1', '_2'))
	...
	Renaming Columns
		...
		try:
			display_rename = self.displayfirsttable.rename (columns={
				"PLACEMENT#":"Placement#",
		...
		try:
			vdx_rename = self.vdx_access_table.rename (
				columns={
		...
		try:
			preroll_rename = self.preroll_access_table.rename (
				columns={
					"PLACEMENT#":"Placement#", "START_DATE":"Start Date", "END_DATE":"End Date",
					"PLACEMENT_NAME":"Placement Name",
					"COST_TYPE":"Cost Type", "UNIT_COST":"Unit Cost",
	...
	Writing Data
		...
		try:
			check_vdx_empty = self.vdx_access_table.empty
			
			if check_vdx_empty is True:
				pass
			else:
				self.logger.info ('Writing VDX Placements to summary for IO - {}'.format(self.config.ioid))
				vdx_write = self.vdx_access_table.to_excel (self.config.writer,
				                                       sheet_name="Summary({})".format (self.config.ioid),
				                                       startcol=3, startrow=len (self.displayfirsttable)+13,
				                                       header=True, index=False)
		...
		try:
			check_preroll_empty = self.preroll_access_table.empty
			
			if check_preroll_empty is True:
				pass
			else:
				self.logger.info ('Writing Preroll Placements to summary for IO - {}'.format(self.config.ioid))
				preroll_write = self.preroll_access_table.to_excel (self.config.writer,
				                                               sheet_name="Summary({})".format (self.config.ioid),
				                                               startcol=3,
				                                               startrow=len (self.displayfirsttable)+len (
					                                               self.vdx_access_table)+18,
					                                               header=True, index=False)
		except(KeyError, AttributeError, ValueError,TypeError) as e:
	...
	Applying formatting
		...
		check_disp_empty = None
		check_vdx_empty = None
		check_preroll_empty = None
		try:
			check_disp_empty = self.displayfirsttable.empty
		except AttributeError as e:
			self.logger.error (str (e)+"No Valid Display placement for IO {}".format (self.config.ioid))
		try:
			check_vdx_empty = self.vdx_access_table.empty
		except AttributeError as e:
			self.logger.error(str(e)+"No Valid VDX placement for IO {}".format (self.config.ioid))
		try:
			check_preroll_empty = self.preroll_access_table.empty
		except AttributeError as e:
			self.logger.error(str(e)+"No Valid Preroll placement for IO {}".format(self.config.ioid))
		...
		number_rows_display = 0
		number_cols_display = 0
		number_rows_vdx = 0
		number_cols_vdx = 0
		number_rows_preroll = 0
		number_cols_preroll = 0
		try:
			number_rows_display = self.displayfirsttable.shape[0]
			number_cols_display = self.displayfirsttable.shape[1]
			number_rows_vdx = self.vdx_access_table.shape[0]
			number_cols_vdx = self.vdx_access_table.shape[1]
			number_rows_preroll = self.preroll_access_table.shape[0]
			number_cols_preroll = self.preroll_access_table.shape[1]
		except AttributeError as e:
			self.logger.error(str(e)+"No valid placement count row and column")
		
		forge_colour_info = workbook.add_format ()
		forge_colour_info.set_bg_color ('#00B0F0')
