...
To create display placements
	...
	def access_Data_KM_Sales_daily(self):
		...
		try:
			...
			self.logger.error (
				str (e)+' No adSize information found for Display placement IO - {}'.format (self.config.ioid))
			pass
		...
		try:
			...
			self.logger.error (str (e)+' No placement by day information found for Display placement IO - {}'.format (
				self.config.ioid))
			pass
	...
	def KM_Sales_daily(self):
		...
		self.logger.info (
			'Adding Delivery Metrices for display placements information of IO {}'.format (self.config.ioid))
		...
		self.logger.info (
			'Adding Deliver Metrices for display placements adsize information of IO {}'.format (self.config.ioid))
		...
		choice_daily_spend = self.daily_sales_third_table["DELIVERED_IMPRESION"]/1000*self.daily_sales_third_table[
			"UNIT_COST"]
			...
			if check_adsize_sales_data is True:
				...
				writing_adsize_data = self.final_adsize.to_excel (self.config.writer,
				                                                  sheet_name="Performance Details".format (
					                                                  self.config.ioid),
				                                                  startcol=1,
				                                                  startrow=len (self.placement_sales_data)+13,
				                                                  index=False,
				                                                  header=True)
			...
			if check_daily_sales_data is True:
				...
				for placement, placement_df in self.final_day_wise.groupby ('Placement# Name', as_index=False):
					...
					worksheet.write_formula (startline, 4,
					                         '=IFERROR(sum(D{}:D{})/sum(C{}:C{}),0)'.format (startRow, startline,
					                                                                         startRow,
					                                                                         startline))
					worksheet.write_formula (startline, 6,
					                         '=IFERROR(sum(H{}:H{})/sum(F{}:F{}),0)'.format (startRow, startline,
					                                                                         startRow,
					                                                                         startline))
