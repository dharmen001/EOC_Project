...
def access_data_summary():
    ...
    KM_pivot_first = pd.pivot_table(read_sql_KM, values=["IMPRESSIONS","ENGAGEMENTS","VWR_CLICK_THROUGHS",
                                    ...
                                    index=["PLACEMENT_ID","PLACEMENT_DESC","DAY_DESC"], columns=
                                    ["METRIC_DESC"], aggfunc=np.sum, margins=True)
    KM_data_daily_new = KM_pivot_first.reset_index()

    print KM_data_daily_new



    #KM_data_daily=KM_pivot_first.stack("METRIC_DESC")
    #print KM_data_daily


    #KM_data_daily_new=KM_data_daily.reset_index()
