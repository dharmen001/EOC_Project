...
class Daily():
	...
	def write_KM_Sales_summary(self):
		...
		if check_daily_sales_data==True:
			...
			for placement, placement_df in final_day_wise.groupby( 'Placement# Name', as_index=False ):
				...
				line_x = "='Standard banner({})'!B{}:B{}".format( self.config.IO_ID, startRow, endRow )
				line_y = "='Standard banner({})'!E{}:E{}".format( self.config.IO_ID, startRow, endRow )
	...
	def formatting_daily(self):
		...
		if check_daily_sales_data==True:
			...
			worksheet.write_formula( row_end,
			                         7, '=SUMIF(B{}:B{},"Subtotal",H{}:H{})'.format( row_start, row_end, row_start,
			                                                                         row_end ), money_fmt )
