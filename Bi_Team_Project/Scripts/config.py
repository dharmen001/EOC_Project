...
class Config(object):
    ...
    def __init__(self, start_date, ioid, end_date):  # start_date,end_date):
        ...
        try:
            ...
            self.logger.error(str(e) + 'Failed to connect to 10.29.20.76: Please Retry for IO: {}').format(self.ioid)
    ...
    def common_columns_summary(self):
        ...
        sql_end_date = "SELECT TO_CHAR(MAX(EDATE),'YYYY-MM-DD') as EDATENEW from TFR_REP.SUMMARY_MV WHERE IO_ID = {}".format(
            self.ioid)

        read_cdb = pd.read_csv("C://BiUiGit//data//mapping//ioIdExpo9ExchangeRates.csv")
        read_discount = pd.read_csv("C://BiUiGit//data//mapping//tfcr//ioIdDiscountRebate.csv")
        ...
        try:
            sdate_edate["Campaign Report date"] = sdate_edate[["Start_Date", "End_Date"]].apply(lambda x: " to ".join(x),
                                                                                                axis=1)
