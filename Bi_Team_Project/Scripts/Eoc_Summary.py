...
class Summary (object):
	...
	merging columns
		...
		try:
			vdx_access_table = vdx_second__table[["PLACEMENT#", "START_DATE", "END_DATE", "PLACEMENT_NAME",
			                                     "COST_TYPE", "UNIT_COST", "PLANNED_COST",
			                                     "BOOKED_IMP#BOOKED_ENG", "Delivered_Engagements",
			                                     "Delivered_Impressions"]]
		except KeyError as e:
			vdx_access_table = vdx_second__table[[]]
			self.logger.error(str(e)+'No valid placements for VDX Creative for IO - {}'.format(self.config.ioid))
	...
	Creating Summary Sheet
		...
		try:
			...
			self.logger.error(str(e)+'No valid vdx placements for IO - {}'.format(self.config.ioid))
