...
Class for VDX Placements
	...
	def connect_tfr_video(self):
		...
		sql_vdx_summary = "select * from (select substr(PLACEMENT_DESC, 1, INSTR(PLACEMENT_DESC, '.',1)-1) as Placement,TO_CHAR(SDATE, 'YYYY-MM-DD') as Start_Date, TO_CHAR(EDATE, 'YYYY-MM-DD') as End_Date, initcap(CREATIVE_DESC)  as Placement_Name,COST_TYPE_DESC as Cost_type, UNIT_COST as Unit_Cost, BUDGET as Planned_Cost,BOOKED_QTY as Booked_Imp_Booked_Eng from  TFR_REP.SUMMARY_MV where (IO_ID = {}) AND (DATA_SOURCE = 'KM') AND (TO_CHAR(SDATE, 'YYYY-MM-DD') >= '{}' AND TO_CHAR(EDATE, 'YYYY-MM-DD') <= '{}') AND CREATIVE_DESC IN(SELECT DISTINCT CREATIVE_DESC FROM TFR_REP.SUMMARY_MV) ORDER BY PLACEMENT_ID) WHERE Placement_Name Not LIKE '%Pre-Roll%' or Placement_Name LIKE '%Pre–Roll%'".format (self.config.ioid,self.config.start_date,self.config.end_date)
		
		sql_vdx_km = "select substr(PLACEMENT_DESC,1,INSTR(PLACEMENT_DESC, '.', 1)-1) as Placement, PRODUCT, sum(IMPRESSIONS) as Impressions,sum(ENGAGEMENTS) as Engagements, sum(DPE_ENGAGEMENTS) as DpeEngaments,sum(ENG_CLICK_THROUGHS) as EngClickthrough, sum(DPE_CLICK_THROUGHS) as DpeClickthrough,sum(VWR_CLICK_THROUGHS) as VwrClickthrough, sum(ENG_TOTAL_TIME_SPENT) as Engtotaltimespent,sum(DPE_TOTAL_TIME_SPENT) as Dpetotaltimespent, sum(VWR_VIDEO_VIEW_0_PC_COUNT) as View0,sum(VWR_VIDEO_VIEW_25_PC_COUNT) as View25, sum(VWR_VIDEO_VIEW_50_PC_COUNT) as View50,sum(VWR_VIDEO_VIEW_75_PC_COUNT) as View75, sum(VWR_VIDEO_VIEW_100_PC_COUNT) as View100,sum(ENG_VIDEO_VIEW_0_PC_COUNT) as Eng0, sum(ENG_VIDEO_VIEW_25_PC_COUNT) as Eng25,sum(ENG_VIDEO_VIEW_50_PC_COUNT) as Eng50, sum(ENG_VIDEO_VIEW_75_PC_COUNT) as Eng75,sum(ENG_VIDEO_VIEW_100_PC_COUNT) as Eng100, sum(DPE_VIDEO_VIEW_0_PC_COUNT) as Dpe0,sum(DPE_VIDEO_VIEW_25_PC_COUNT) as Dpe25, sum(DPE_VIDEO_VIEW_50_PC_COUNT) as Dpe50,sum(DPE_VIDEO_VIEW_75_PC_COUNT) as Dpe75, sum(DPE_VIDEO_VIEW_100_PC_COUNT) as Dpe100,sum(DPE_INTERACTIVE_ENGAGEMENTS) as DpeIntractiveEngagements, sum(ENG_INTERACTIVE_ENGAGEMENTS) as EngIntractiveEngagements,sum(CPCV_COUNT) as completions from TFR_REP.KEY_METRIC_MV WHERE IO_ID = {} AND TO_CHAR(TO_DATE(DAY_DESC, 'MM/DD/YYYY'),'YYYY-MM-DD') BETWEEN '{}' AND '{}' GROUP BY PLACEMENT_ID, PLACEMENT_DESC,PRODUCT ORDER BY PLACEMENT_ID".format (self.config.ioid,self.config.start_date,self.config.end_date)
		
		sql_adsize_km = "SELECT substr(PLACEMENT_DESC,1,INSTR(PLACEMENT_DESC, '.', 1)-1) as Placement,PRODUCT, MEDIA_SIZE_DESC as AdSize,sum(IMPRESSIONS) as Impressions, sum(ENGAGEMENTS) as Engagements, sum(DPE_ENGAGEMENTS) as DpeEngagements, sum(ENG_CLICK_THROUGHS) as EngClickthroughs, sum(DPE_CLICK_THROUGHS) as DpeClickthroughs, sum(VWR_CLICK_THROUGHS) as VwrClickthroughs,sum(VWR_VIDEO_VIEW_100_PC_COUNT) as View100, sum(ENG_VIDEO_VIEW_100_PC_COUNT) as Eng100,sum(DPE_VIDEO_VIEW_100_PC_COUNT) as Dpe100, sum(ENG_TOTAL_TIME_SPENT) as Engtotaltimespent,sum(DPE_TOTAL_TIME_SPENT) as Dpetotaltimespent, sum(ENG_INTERACTIVE_ENGAGEMENTS) as EngIntractiveEngagements, sum(DPE_INTERACTIVE_ENGAGEMENTS) as DpeIntractiveEngagements,sum(CPCV_COUNT) as completions FROM TFR_REP.ADSIZE_KM_MV WHERE IO_ID = {} AND TO_CHAR(DAY_DESC, 'YYYY-MM-DD') BETWEEN '{}' AND '{}' GROUP BY PLACEMENT_ID, PLACEMENT_DESC, MEDIA_SIZE_DESC,PRODUCT ORDER BY PLACEMENT_ID".format(self.config.ioid,self.config.start_date,self.config.end_date)
		
		sql_video_km = "SELECT substr(PLACEMENT_DESC,1,INSTR(PLACEMENT_DESC, '.', 1)-1) as Placement,PRODUCT,FEV_INT_VIDEO_DESC as Videoname,sum(VWR_VIDEO_VIEW_0_PC_COUNT) as View0,sum(VWR_VIDEO_VIEW_25_PC_COUNT) as View25,sum(VWR_VIDEO_VIEW_50_PC_COUNT) as View50,sum(VWR_VIDEO_VIEW_75_PC_COUNT) as View75,sum(VWR_VIDEO_VIEW_100_PC_COUNT) as View100,sum(ENG_VIDEO_VIEW_0_PC_COUNT) as Eng0,sum(ENG_VIDEO_VIEW_25_PC_COUNT) as Eng25,sum(ENG_VIDEO_VIEW_50_PC_COUNT) as Eng50,sum(ENG_VIDEO_VIEW_75_PC_COUNT) as Eng75,sum(ENG_VIDEO_VIEW_100_PC_COUNT) as Eng100,sum(DPE_VIDEO_VIEW_0_PC_COUNT) as Dpe0,sum(DPE_VIDEO_VIEW_25_PC_COUNT) as Dpe25,sum(DPE_VIDEO_VIEW_50_PC_COUNT) as Dpe50,sum(DPE_VIDEO_VIEW_75_PC_COUNT) as Dpe75,sum(DPE_VIDEO_VIEW_100_PC_COUNT) as Dpe100 FROM  TFR_REP.VIDEO_DETAIL_MV WHERE IO_ID = 599207 AND TO_CHAR(DAY_DESC, 'YYYY-MM-DD') BETWEEN '2018-04-01' AND '2018-04-30' GROUP BY PLACEMENT_ID,PLACEMENT_DESC,PRODUCT, FEV_INT_VIDEO_DESC ORDER BY PLACEMENT_ID".format(self.config.ioid,self.config.start_date,self.config.end_date)
		
		sql_video_player_intraction = "SELECT PRODUCT,sum(VWR_MUTE) as Vwrmute,sum(VWR_UNMUTE) as Vwrunmute,sum(VWR_PAUSE) as Vwrpause, sum(VWR_REWIND) as Vwrrewind, sum(VWR_RESUME) as Vwrresume,sum(VWR_REPLAY) as Vwrreplay, sum(VWR_FULL_SCREEN) as Vwrfullscreen,sum(ENG_MUTE) as Engmute, sum(ENG_UNMUTE) as Engunmute, sum(ENG_PAUSE) as Engpause, sum(ENG_REWIND) as Engrewind,sum(ENG_RESUME) as Engresume, sum(ENG_REPLAY) as Engreplay,sum(ENG_FULL_SCREEN) as Engfullscreen, sum(DPE_MUTE) as Dpemute,sum(DPE_UNMUTE) as Dpeunmute, sum(DPE_PAUSE) as Dpepause, sum(DPE_REWIND) as Dperewind, sum(DPE_RESUME) as Dperesume,sum(DPE_REPLAY) as Dpereplay,sum(DPE_FULL_SCREEN) as Dpefullscreen FROM TFR_REP.VIDEO_DETAIL_MV WHERE IO_ID = {} AND TO_CHAR(DAY_DESC, 'YYYY-MM-DD') BETWEEN '{}' AND '{}' GROUP BY PRODUCT ORDER BY PRODUCT".format (self.config.ioid,self.config.start_date,self.config.end_date)
		
		sql_ad_intraction = "select * from(select product, blaze_action_type_desc, blaze_tag_name_desc,sum(decode(product, 'InStream', vwr_interaction, eng_interaction)) as interaction from Interaction_detail_mv where io_id = {} AND TO_CHAR(DAY_DESC, 'YYYY-MM-DD') BETWEEN '{}' AND '{}'  group by product, blaze_action_type_desc,blaze_tag_name_desc order by product, blaze_action_type_desc, blaze_tag_name_desc) pivot (sum(interaction) for blaze_action_type_desc in('Click-thru' Clickthru, 'Interaction' Interaction)) order by product,blaze_tag_name_desc".format (self.config.ioid,self.config.start_date,self.config.end_date)
		
		sql_click_throughs = "SELECT PRODUCT,BLAZE_ACTION_TYPE_DESC,BLAZE_TAG_NAME_DESC, sum(VWR_INTERACTION) as Vwrclickintraction, sum(ENG_INTERACTION) as Engclickintraction, sum(DPE_INTERACTION) as Dpeclickintraction FROM TFR_REP.INTERACTION_DETAIL_MV WHERE IO_ID = {} AND TO_CHAR(DAY_DESC, 'YYYY-MM-DD') BETWEEN '{}' AND '{}' AND BLAZE_ACTION_TYPE_DESC = 'Click-thru' GROUP BY PRODUCT,BLAZE_ACTION_TYPE_DESC,BLAZE_TAG_NAME_DESC ORDER BY PRODUCT".format(self.config.ioid,self.config.start_date,self.config.end_date)
		
		sql_vdx_day_km = "SELECT substr(PLACEMENT_DESC,1,INSTR(PLACEMENT_DESC, '.', 1)-1) as Placement, PRODUCT,TO_CHAR(TO_DATE(DAY_DESC, 'MM/DD/YYYY'),'YYYY-MM-DD') as Day,sum(IMPRESSIONS) as Impressions, sum(ENGAGEMENTS) as Engagements, sum(DPE_ENGAGEMENTS) as Dpeengagements, sum(VWR_VIDEO_VIEW_100_PC_COUNT) as View100, sum(CPCV_COUNT) as completions, sum(ENG_VIDEO_VIEW_100_PC_COUNT) as Eng100,sum(DPE_INTERACTIVE_ENGAGEMENTS) as DpeIntractiveEngagements, sum(DPE_VIDEO_VIEW_100_PC_COUNT) as Dpe100 FROM TFR_REP.KEY_METRIC_MV WHERE IO_ID = {} AND TO_CHAR(TO_DATE(DAY_DESC, 'MM/DD/YYYY'),'YYYY-MM-DD') BETWEEN '{}' AND '{}' GROUP BY PLACEMENT_ID, PLACEMENT_DESC, DAY_DESC,PRODUCT ORDER BY PLACEMENT_ID".format(self.config.ioid,self.config.start_date,self.config.end_date)
	...
	def access_vdx_columns(self):
		...
		# placement_by_km_video_summary = reduce(lambda left, right: pd.merge(left, right, on=['PLACEMENT',
		# 'PRODUCT']),
		# placement_by_km_video)
		...
		# exit()
		# print (placement_by_video_summary_new)
		# mask22 = (placement_by_video_summary_new.PRODUCT.str.upper ()=='DISPLAY') & (
		# placement_by_video_summary_new.COST_TYPE=='CPE')
		...
		'''intractionclickgrouping = pd.pivot_table(self.read_sql_click_throughs,index = ['PRODUCT'],
												 values=["VWRCLICKINTRACTION","ENGCLICKINTRACTION",
												 "DPECLICKINTRACTION"],
												 columns=["BLAZE_TAG_NAME_DESC"],aggfunc=np.sum,fill_value=0)

		intractionclickgroupingnew = intractionclickgrouping.reset_index()

		intractionclickgroupingnew_r = intractionclickgroupingnew.loc[:,["PRODUCT","VWRCLICKINTRACTION",
		"ENGCLICKINTRACTION","DPECLICKINTRACTION"]]

		intractionadinteractiongrouping  = pd.pivot_table(self.read_sql_ad_intraction,index = ['PRODUCT'],
														  values=["VWRADINTRACTION","ENGADINTRACTION",
														  "DPEADINTRACTION"]
														  ,columns=["BLAZE_TAG_NAME_DESC"],aggfunc=np.sum,fill_value=0)

		intractionadinteractiongroupingnew = intractionadinteractiongrouping.reset_index()

		intractionadinteractiongrouping_r = intractionadinteractiongroupingnew.loc[:,["PRODUCT","VWRADINTRACTION",
		"ENGADINTRACTION",
																					  "DPEADINTRACTION"]]'''
...
Main Function
		...
		if self.read_sql_vdx_summary.empty:
